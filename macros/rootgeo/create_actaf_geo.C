...
void create_actaf_geo(const char *geoTag = "v25") {
  ...
  std::vector<TGeoTubeSeg *> rings = {
      new TGeoTubeSeg("Ring1_section",
                      2.0,               // rmin
                      4.6,               // rmax
      ...
      new TGeoTubeSeg("Ring2_section", 4.6, 8.5, anode_length / 2., 0.,
                      360. / 8.),

      new TGeoTubeSeg("Ring3_section", 8.6, 12.5, anode_length / 2., 0.,
                      360. / 8.),

      new TGeoTubeSeg("Ring4_section", 12.6, 16.5, anode_length / 2., 0.,
                      360. / 8.),

      new TGeoTubeSeg("Ring5_section", 16.6, 20.5, anode_length / 2., 0.,
                      360. / 8.),

      new TGeoTubeSeg("Ring6_section", 20.6, 24.5, anode_length / 2., 0.,
                      360. / 6.),

      new TGeoTubeSeg("Ring7_section", 24.6, 28.5, anode_length / 2., 0.,
                      360. / 7.),

      new TGeoTubeSeg("Ring8_section", 28.6, 30.9, anode_length / 2., 0.,
                      360. / 6.)};
  ...
  for (int r = 0; r < 8; r++) {
    for (int s = 0; s < nb_pads_ring[r]; s++) {
      ...
      Section->AddNode(section, 0,
                       new TGeoCombiTrans(0., 0., -anode_length / 2. - 1, rot));
